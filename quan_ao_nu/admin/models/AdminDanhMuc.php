...
class AdminDanhMuc{
 ...
 public function insert_dm(danh_mucs $danh_mucs){
    try{
    $sql="INSERT INTO danh_mucs ( ten_danh_muc, mo_ta) VALUES ('$danh_mucs->ten_danh_muc','$danh_mucs->mo_ta')";
...
public function updateDM($id,danh_mucs $danh_mucs){
    try{
       $sql="UPDATE danh_mucs SET `id`='$danh_mucs->id', `ten_danh_muc`='$danh_mucs->ten_danh_muc',`mo_ta`='$danh_mucs->mo_ta' WHERE id=$id";
       $data = $this->pdo->exec($sql);
       if($data ===1 || $data===0){
        return "success";
       }
